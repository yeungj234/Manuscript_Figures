...
# ATAC-seq: plot motifs that appear to be drug specific in Down peaks i.e. enriched in Doxo at any timepoint but not Palbo & vice versa 
```{r}
# get IDs of drug specific motifs
drug_unique_motifs <- c(row.names(Doxo_DownPeaks_AME_mat)[!row.names(Doxo_DownPeaks_AME_mat) %in% row.names(Palbo_DownPeaks_AME_mat)], row.names(Palbo_DownPeaks_AME_mat)[!row.names(Palbo_DownPeaks_AME_mat) %in% row.names(Doxo_DownPeaks_AME_mat)]) 

# create matrix of -log10(padj) values of each motif ID for each DESeq comparison of drug specific motifs
drug_enriched_motifs_Down_Peaks_AME_mat <- create_AME_peaks_matrix(annoDownPeaks_AME_df[annoDownPeaks_AME_df$motif_ID %in% drug_unique_motifs, ])
drug_enriched_motifs_Down_Peaks_AME_mat <- drug_enriched_motifs_Down_Peaks_AME_mat[order(rowSums(drug_enriched_motifs_Down_Peaks_AME_mat), decreasing=T), ]

pheatmap(drug_enriched_motifs_Down_Peaks_AME_mat, cluster_rows = F, cluster_cols=F, color = colorRampPalette(c("white",  "#FDBB84", "#B30000"))(100), breaks = seq(0, 10,length.out = 100), main="all Down: enriched motifs")
...
```{r}
Palbo2_IPA_TFs <- read_delim("/lustre/fs4/risc_lab/scratch/jyeung/Manuscript_Figures/Figure_Drug_distinct_responses_and_SASP/IPA_files_kmeans7_clusters/output/sigGenes_log2FC/databases/Palbo2_sigLFC_Comparison_Analysis_IPA_upstream_regs_transcription_regulators_database.txt", 
    delim = "\t", escape_double = FALSE, 
    trim_ws = TRUE, skip = 2)
mat <- IPA_Compare_upstream_regs_mat$Palbo2[order(rowSums(IPA_Compare_upstream_regs_mat$Palbo2[ ,6:10]), decreasing=T), ]
filt_mat <- mat[row.names(mat) %in% Palbo2_IPA_TFs$`Upstream Regulator`, ]
top_regs <- c(row.names(head(filt_mat)), row.names(tail(filt_mat)))

filt_mat <- filt_mat[row.names(filt_mat) %in% row.names(Palbo_UpPeaks_AME_mat), ]
```
