...
top65_motifs <- unique(c(row.names(Doxo_UpPeaks_AME_mat)[1:65], row.names(Palbo_UpPeaks_AME_mat)[1:65]))
top65_motifs_Palbo_Doxo_AME_Mat <- create_AME_peaks_matrix(annoUpPeaks_AME_df[annoUpPeaks_AME_df$motif_ID %in% top65_motifs, ])

top65_motifs_Palbo_Doxo_AME_Mat <- top65_motifs_Palbo_Doxo_AME_Mat[order(rowSums(top65_motifs_Palbo_Doxo_AME_Mat), decreasing=T), ]# order by overall most to least enriched motifs
...
pheatmap(top65_motifs_Palbo_Doxo_AME_Mat , cluster_rows = T, cluster_cols=F, color = colorRampPalette(c("white",  "#FDBB84", "#B30000"))(100), breaks = seq(0, 200,length.out = 100), main="top 50 enriched motifs in up peaks")
dev.off()
