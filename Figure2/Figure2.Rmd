---
title: Figure 2: Global transcriptome and chromatin accessibility is dynamic in both types of therapy induced senescence
author: "Joanna Yeung"
date: '2025-01-22'
...
library(pheatmap)
```
# load in gene annotation database 
```{r}
load("/lustre/fs4/risc_lab/scratch/jyeung/LS_PDvsDoxo_1/RNAseq/post_kallisto_analysis/workspaces/genes_anno_type.RData")
```
```{r, eval=F}
# CODE TO GET genes_anno_type.RData
# Get latest gene annotation. Since RStudio on the cluster is unable to download the latest version of EnsDb.Hsapiens package, I had to manually download from ensembl and NCBI and save it on to my store. 
# 9-11-2022: since anything later than v79 is not available in this version of R studio, I downloaded v107 from http://ftp.ensembl.org/pub/release-107/gff3/homo_sapiens/
...
#gsub('.*\\HGNC:|.*\\Acc:','', Ens_genes_v107$description) # remove anything before "HGNC:" or "Acc:", only keep the entrez ID numbers

# 9-11-2022: ensembl database does not have NCBI entrez ID format, so the latest version of hg38 from NCBI is downloaded here. 
...
# load in msigdb databases workspaces, to be used for GO enrichment analysis. 
```{r}
...
```
```{r, eval=F}
# CODE TO GET msigdbr_df_andt2g_workspace.RData
...
```{r, eval=F}
# CODE TO IMPORT KALLISTO PSEUDOCOUNTS
...
# include samples from previous harvest. 
...
```{r}
# load in kallisto counts workspace from both biological replicates
...
keep <- rowSums(counts(RNAddsallLRT)) >= 50 # filter out rows with rowsum less than 50 to remove genes that aren't being expressed across samples. 
...
row.names(sigMat) <- make.unique(ifelse(genes_anno_type[match(row.names(sigMat), genes_anno_type$ensembl_id), ]$gene_id %in% NA, row.names(sigMat), genes_anno_type[match(row.names(sigMat), genes_anno_type$ensembl_id), ]$gene_id) )
...
clusterDF$SASP <- ifelse(clusterDF$gene_id %in% SASP_all, "Yes", "No")

# annotate which genes belong to the cell cycle based on whether or not they are found in the following MSigDb datasets. 
clusterDF$Cell_Cycle <- ifelse(clusterDF$gene_id %in% c(msigdbr_df_H[msigdbr_df_H$gs_name %in% c("HALLMARK_E2F_TARGETS", "HALLMARK_G2M_CHECKPOINT"), ]$gene_symbol, msigdbr_df_REACT[msigdbr_df_REACT$gs_name %in% c("REACTOME_CELL_CYCLE", "REACTOME_CELL_CYCLE_CHECKPOINTS", "REACTOME_CELL_CYCLE_MITOTIC"), ]$gene_symbol, msigdbr_df_KEGG[msigdbr_df_KEGG$gs_name %in% "KEGG_CELL_CYCLE", ]$gene_symbol), "Yes", "No")

# set row names as gene ids 
row.names(clusterDF) <- make.unique(as.character(clusterDF$gene_id))

drug_cat_list <- list(sigMat[row.names(sigMat) %in% clusterDF[clusterDF$Clusters %in% "2", ]$gene_id, ], 
                      sigMat[row.names(sigMat) %in% clusterDF[clusterDF$Clusters %in% "5", ]$gene_id, ], 
                      sigMat[row.names(sigMat) %in% clusterDF[clusterDF$Clusters %in% "7", ]$gene_id, ], 
                      sigMat[row.names(sigMat) %in% clusterDF[clusterDF$Clusters %in% "1", ]$gene_id, ], 
                      sigMat[row.names(sigMat) %in% clusterDF[clusterDF$Clusters %in% "4", ]$gene_id, ],
                      sigMat[row.names(sigMat) %in% clusterDF[clusterDF$Clusters %in% "6", ]$gene_id, ],
                      sigMat[row.names(sigMat) %in% clusterDF[clusterDF$Clusters %in% "3", ]$gene_id, ]
)
  ...
  # Check if the subset has at least 2 rows before clustering
  if (nrow(ordered_sigMat[[i]]) >= 2) {
    ...
    # Skip clustering and keep as is
    message(paste("Skipping clustering for category", i, "due to insufficient rows"))
  }
}
...
```
```{r, eval=F}
# CODE TO import master peaks after iterative peak filtering
setwd("/lustre/fs4/risc_lab/scratch/jyeung/LS_PDvsDoxo_2/ATACseq/Playing_with_Our_Peak_Calling_Settings_5192024/iterative_peak_filtering/output_nonredundant_masterpeaks")
masterpeaks <- lapply(dir(pattern="FW500"), import)
names(masterpeaks) <- gsub("OurSettings_FW500_", "", gsub("nonredundant_peaks_", "", dir(pattern="FW500")))
names(masterpeaks) <- gsub(".gtf", "", names(masterpeaks))
```

```{r, eval=F}
### CODE TO COUNT READS UNDER PEAKS
setwd("/lustre/fs4/risc_lab/scratch/jyeung/LS_PDvsDoxo_2/ATACseq/3022023_run_Deep_Justin_scripts/atac_4_bam_markduplicates")
bamDir <- dir(pattern="final.bam$") # get names of bam files
fragCounts_rep2 <- chromVAR::getCounts(bamDir, masterpeaks$pval0.2_peakfilt, paired=TRUE, by_rg=FALSE, format="bam") # count reads of each bam file under peaks
# convert fragCounts to numeric matrix of fragment counts under peaks
fragCounts_rep2 <- as.matrix(SummarizedExperiment::assays(fragCounts_rep2)[[1]]) # convert fragment Counts into counts matrix
colnames(fragCounts_rep2) <- gsub("Quies", "day3", colnames(fragCounts_rep2))
colnames(fragCounts_rep2) <- gsub("_final.bam", "", colnames(fragCounts_rep2))
# change row names to bin coordinates
rownames(fragCounts_rep2) <- masterpeaks$pval0.2_peakfilt$name

save(fragCounts_rep2, file="/lustre/fs4/risc_lab/scratch/jyeung/LS_PDvsDoxo_2/ATACseq/post_peakcalling_analysis/06222024/workspaces/fragCounts_rep2.RData")

setwd("/lustre/fs4/risc_lab/scratch/jyeung/LS_PDvsDoxo_1/ATACseq/5182022_run_Deep_Justin_scripts/4markduplicates_output")
bamDir_rep1 <-dir(pattern="final.bam$") 
bamDir_rep1 <- bamDir_rep1[-5]
fragCounts_rep1 <- chromVAR::getCounts(bamDir_rep1, masterpeaks$pval0.2_peakfilt, paired=TRUE, by_rg=FALSE, format="bam")
colnames(fragCounts_rep1) <- gsub("_final.bam", "", colnames(fragCounts_rep1))
# change row names to bin coordinates
rownames(fragCounts_rep1) <- masterpeaks$pval0.2_peakfilt$name

fragCounts_rep1 <- as.matrix(SummarizedExperiment::assays(fragCounts_rep1)[[1]])

fragCounts_all <- cbind(fragCounts_rep2, fragCounts_rep1)

save(fragCounts_rep1, file="/lustre/fs4/risc_lab/scratch/jyeung/LS_PDvsDoxo_2/ATACseq/post_peakcalling_analysis/06222024/workspaces/fragCounts_rep1.RData")

save(fragCounts_all, file="/lustre/fs4/risc_lab/scratch/jyeung/LS_PDvsDoxo_2/ATACseq/post_peakcalling_analysis/06222024/workspaces/fragCounts_all.RData")
```
...
```
# save workspace because most of the files generated here are inputs for other analysis. 
```{r}
save.image("/lustre/fs4/risc_lab/scratch/jyeung/Manuscript_Figures/Figure2/workspace.RData")
```
---------------------------------------------------------------------------------------
# *FIGURES*
```{r}
# set output directory for generating plots
outdir <- "/lustre/fs4/risc_lab/scratch/jyeung/Manuscript_Figures/Figure2"
```

# plots for RNA-seq & ATAC-seq sample distance matrix
```{r}
pdf(paste0(outdir, "/sampleDistMatrix.pdf"), width=4.25, height=4)
...
```{r}
pdf(paste0(outdir,"/PCA.pdf"), width=4, height=2)
...
```{r}
pdf(paste0(outdir,"/RNA_diff_genes_Mat.pdf"), width = 4.2, height = 10)
pheatmap(ordered_sigMat, scale = "row", annotation_row = clusterDF[ ,!colnames(clusterDF) %in% "gene_id"], show_rownames = FALSE, show_colnames = F,
...
generatemergedDiff_hm(matrix=mergedDiff_hm_rmOutlier$DownPalbo.DownDoxo, breaks = seq(-3, 3,length.out = 100), output_file=paste0(outdir,"/ATAC_all_Down_Peaks_Peaks_Mat.pdf"), hm_colors=hm_cols_ATAC, plot_Title = paste0("all Down Peaks:", nrow(mergedDiff_hm_rmOutlier$DownPalbo.DownDoxo)), annoCol=annoCol, DoxoGR=mergedDiffGR$allDown_Doxo, PalboGR=mergedDiffGR$allDown_Palbo)
```

# ATAC-seq: merge technical replicates into 1 sample to make the heatmap look like overwhelming & stripy. 
```{r}
dds_filt$group <- factor(paste0(dds_filt$Condition, dds_filt$Biorep))
#dds_filt_collapsed <- collapseReplicates(dds_filt, dds_filt$group)
...
annoCol_collapsed <- colData(dds_filt_collapsed)[ ,c("Condition", "Treatment", "Timepoint")]
annoCol_collapsed <- as.data.frame(annoCol_collapsed[c(1,9,2,4,6,10,3,5,7,8), ])

collapsed_hm <- list(UpPalbo.UpDoxo=vstdds_matrix_collapsed[row.names(vstdds_matrix_collapsed) %in% row.names(mergedDiff_hm_rmOutlier$UpPalbo.UpDoxo), ], DownPalbo.DownDoxo=vstdds_matrix_collapsed[row.names(vstdds_matrix_collapsed) %in% row.names(mergedDiff_hm_rmOutlier$DownPalbo.DownDoxo), ])

pdf_dir <- "/lustre/fs4/risc_lab/scratch/jyeung/Manuscript_Figures/Figure2"

generatemergedDiff_hm(matrix=collapsed_hm$UpPalbo.UpDoxo, breaks = seq(-3, 3,length.out = 100), output_file=paste0(pdf_dir, "/ATAC_all_Up_Peaks_collapsedMat.pdf"), hm_colors=hm_cols_ATAC, plot_Title = paste0("all Up Peaks: ", nrow(mergedDiff_hm_rmOutlier$UpPalbo.UpDoxo)), annoCol=annoCol, DoxoGR=mergedDiffGR$allUp_Doxo, PalboGR=mergedDiffGR$allUp_Palbo)

generatemergedDiff_hm(matrix=collapsed_hm$DownPalbo.DownDoxo, breaks = seq(-3, 3,length.out = 100), output_file=paste0(pdf_dir, "/ATAC_all_Down_Peaks_collapsedMat.pdf"), hm_colors=hm_cols_ATAC, plot_Title = paste0("all Down Peaks: ", nrow(mergedDiff_hm_rmOutlier$DownPalbo.DownDoxo)), annoCol=annoCol, DoxoGR=mergedDiffGR$allDown_Doxo, PalboGR=mergedDiffGR$allDown_Palbo)
```
