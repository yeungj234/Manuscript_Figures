...
p1 <- gseaplot(GSEA_Results_H$`Doxo14 vs Cycling0`, geneSetID = "TNFA SIGNALING VIA NFKB", by="runningScore", title = "day 14 Doxo vs Cycling: TNFA SIGNALING VIA NFKB", color.line="#B3C543", color="#B3C543", color.vline="#B3C543")+theme_classic()+ylim(-0.05, 0.8)

p2 <- gseaplot(GSEA_Results_H$`Palbo14 vs Cycling0`, geneSetID = "TNFA SIGNALING VIA NFKB", by="runningScore", title = "day 14 Palbo vs Cycling: TNFA SIGNALING VIA NFKB", color="#E0ADCD", color.line="#E0ADCD", color.vline="#E0ADCD")+theme_classic()+ylim(-0.05, 0.8)

p3 <- gseaplot(GSEA_Results_H$`Doxo14 vs Doxo3`, geneSetID = "TNFA SIGNALING VIA NFKB", by="runningScore", title = "day 14 Doxo vs day 3 Doxo: TNFA SIGNALING VIA NFKB", color.line="#B3C543", color="#B3C543", color.vline="#B3C543")+theme_classic()+ylim(-0.05, 0.8)

p4 <- gseaplot(GSEA_Results_H$`Palbo14 vs Palbo3`, geneSetID = "TNFA SIGNALING VIA NFKB", by="runningScore", title = "day 14 Palbo vs day 3 Palbo: TNFA SIGNALING VIA NFKB", color="#E0ADCD", color.line="#E0ADCD", color.vline="#E0ADCD")+theme_classic()+ylim(-0.05, 0.8)
...
create_violin_plot(
  ...
  plot_title = "RNA: log2FC of NF-kB target genes"
) # after talking with Justin, we both agreed it looks better to include day 3 controls and plot relative to Cycling to show the increase over time in both drugs. 
dev.off()
```
...
create_AME_dotplots(annoUpPeaks_AME_df[grepl("NFKB|REL", annoUpPeaks_AME_df$motif_ID), ], Title="NFkB Motifs enriched in Up peaks", fill_limit=90, size_limit=15)
dev.off()

# note: initially, I considered only showing NFKB related motif enrichment nearby shared up genes but NFkB is also enriched nearby Palbo enriched and Doxo enriched upregulated genes, not just shared. Therefore, I think it makes sense to just show the motif enrichment in all up peaks since the enrichment is not dependent/exclusive on how close it is to shared up genes
```
